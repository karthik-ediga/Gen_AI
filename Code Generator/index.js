...
// exec is a function. Specifically, it is a Higher-Order Function 
// because it accepts another function (a callback) as an argument.
...
//windows or mac etc for llm to know for commands as there are diff for diff os
const platform = os.platform();
//It takes the exec function from child_process (which uses a callback) and 
// transforms it into a new function (execute) that returns a Promise.
// Callbacks can lead to "callback hell" if you have multiple shell 
// commands to run in a row. Using util.promisify allows you to use async/await, 
// making your code read synchronously and look much cleaner.

//It transforms the callback-based exec into a Promise-based function.
...
const blockedCommands = ["rm", "shutdown", "del", "rmdir", "format"];
...
//no toolfunctions calling only single function directly
